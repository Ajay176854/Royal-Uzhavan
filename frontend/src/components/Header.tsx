...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-sm flex flex-col w-full border-b border-gray-100">
      ...
      <div className="container mx-auto px-4 xl:px-8 py-3 md:py-4 flex items-center justify-between gap-6">
        ...
        <Link to="/" className="flex shrink-0 items-center gap-3 md:gap-4 group">
          <div className="relative">
            <img src={logoImg} alt="Royal Uzhavan Logo" className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 object-cover rounded-full shadow-md border-[3px] border-white group-hover:shadow-lg transition-all duration-300 shrink-0" />
            <div className="absolute -bottom-0.5 -right-0.5 bg-white rounded-full p-1 shadow-sm border border-gray-100">
              <Leaf className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#86B841]" strokeWidth={2.5} />
            </div>
          </div>
          <div className="flex flex-col text-left justify-center mt-1">
            <span className="text-[#1B4332] text-2xl md:text-3xl lg:text-[32px] font-serif font-bold tracking-tight leading-[0.85]">Royal</span>
            <span className="text-[#C9A227] text-2xl md:text-3xl lg:text-[32px] font-serif font-bold tracking-tight leading-[0.85]">Uzhavan</span>
            <span className="text-[9px] md:text-[10px] lg:text-[11px] text-[#5c8a2b] font-black tracking-[0.2em] uppercase mt-1.5 md:mt-2 flex items-center gap-1.5">
              <span className="w-4 md:w-6 h-[2px] bg-[#86B841]/40 rounded-full"></span>
              Traditional Farms 🌾
            </span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex flex-1 justify-center items-center gap-2 lg:gap-4 xl:gap-6 text-[9px] lg:text-[10px] xl:text-xs font-bold uppercase tracking-wider text-gray-700 whitespace-nowrap">
          ...
          <div className="group">
            ...
            <div className="absolute top-[100%] left-0 w-full bg-white shadow-2xl border-t border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 cursor-default">
              <div className="container mx-auto px-4 xl:px-8 py-8 grid grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-10">
          ...
          <div className="relative group">
            <button className="flex items-center gap-1 hover:text-[#1B4332] transition-colors py-2 px-1">
              Policy <ChevronDown className="w-3.5 h-3.5" strokeWidth={2.5} />
            </button>
            <div className="absolute top-full left-0 bg-white shadow-xl border border-gray-100 rounded-lg py-3 min-w-[180px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 translate-y-1 group-hover:translate-y-0 normal-case tracking-normal">
              <Link to="/policies?tab=refund" className="block px-6 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-50 hover:text-[#1B4332]">Refund Policy</Link>
              <Link to="/policies?tab=terms" className="block px-6 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-50 hover:text-[#1B4332]">Terms of Service</Link>
              <Link to="/policies?tab=privacy" className="block px-6 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-50 hover:text-[#1B4332]">Privacy Policy</Link>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 lg:gap-5 shrink-0 ml-auto md:ml-0">
          ...
          <button
            ...
            className="relative bg-[#86B841] text-white w-11 h-11 rounded-full hidden md:flex items-center justify-center hover:bg-[#729c36] transition-colors shadow-sm"
          >
          ...
          {/* WhatsApp Icon */}
          <a href="https://wa.me/918072864890" target="_blank" rel="noopener noreferrer" className="hidden sm:flex items-center justify-center w-11 h-11 rounded-full border border-gray-200 bg-gray-50 hover:bg-[#25D366] hover:text-white hover:border-[#25D366] text-gray-600 transition-all shadow-sm">
            <MessageCircle className="w-5 h-5" strokeWidth={2} />
          </a>

          {/* Mobile Menu Toggle (Moved to Right) */}
          <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden text-gray-800 p-1">
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-black/20 backdrop-blur-sm z-[100] transition-opacity duration-300 md:hidden ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsMobileMenuOpen(false)}
      />

      {/* Mobile Menu Drawer */}
      <div className={`fixed top-0 right-0 h-[100dvh] w-[80%] max-w-[300px] bg-white shadow-2xl z-[100] flex flex-col transform transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto py-2 flex flex-col">
          <Link to="/shop" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50 flex items-center justify-between">
            Shop Products <ChevronDown className="w-4 h-4 -rotate-90 text-gray-400" />
          </Link>
          <Link to="/our-farms" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50">About Us</Link>
          <Link to="/policies?tab=shipping" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50">Shipping & Bulk Orders</Link>
          <Link to="/policies?tab=refund" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50">Policies</Link>
          <Link to="/blog" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50">Blog</Link>
          <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-4 font-bold text-gray-800 border-b border-gray-50">Contact Us</Link>
          <Link to="/account" onClick={() => setIsMobileMenuOpen(false)} className="px-6 py-6 font-black text-[#1B4332] uppercase tracking-wider text-sm mt-auto bg-gray-50 text-center">
            Login / Register
          </Link>
        </div>
      </div>
