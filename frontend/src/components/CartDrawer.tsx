...
export default function CartDrawer() {
  const { isCartOpen, setIsCartOpen, cartItems, cartSubtotal, updateQuantity, removeFromCart } = useCart();
  const navigate = useNavigate();

  // We render the drawer even when closed to allow the slide-out animation to play
  // but we control its position using translate-x.
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-gray-100 bg-gray-50">
            <div className="flex justify-between items-end mb-4">
              ...
              <span className="text-xl font-bold text-[#0B4D26]">₹{cartSubtotal.toLocaleString('en-IN')}</span>
            </div>
